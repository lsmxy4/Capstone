...
export default function ExerciseInfo() {
  ...
  return <><style>{dashboardStyles}</style><div className="dashboard exercise-page dashboard-exercise"><Sidebar /><main className="content" id="top">
    <header className="welcome"><div><span className="eyebrow">MOVE AT YOUR OWN PACE</span><h1>운동 정보</h1><p><Icon name="activity" size={12} /> 운동별 추천 정보와 주의사항을 확인하세요</p></div><div className="header-actions"><a href="/dashboard">대시보드로 돌아가기 ↗</a></div></header><LocationStatus location={location} />
    ...
    <section className="panel movement-map-panel"><div className="title-row"><h2><span className="title-icon"><Icon name="map" size={15} /></span>내 이동 경로</h2><span className="good">{dailyRoute.selectedDate === dailyRoute.today ? '오늘' : dailyRoute.selectedDate}</span></div>
      ...
      {todaySelected && movement.accuracyMeters != null && movement.accuracyMeters > 100 && <p className="movement-notice" role="status">현재 위치의 정확도가 약 {Math.round(movement.accuracyMeters)}m입니다. 부정확한 새 위치는 지도에 표시하거나 저장하지 않습니다. 정확도가 100m 이내로 개선되면 기록을 재개합니다.</p>}
